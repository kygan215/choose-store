...
约 30 名用户使用时，采用 Node/Vinext + PostgreSQL + Redis 队列 + Nginx 的 Docker Compose 正式架构。当前回源端口通过 `HTTP_PORT` 配置，默认使用 HTTP 7090；443 和域名由后续网关配置承担。完整步骤见 `docs/单服务器正式部署指南.md`。
...
正式部署版按登录账号隔离任务数据。任务列表、门店详情、AI 分析、任务控制和导出接口均在服务端校验 `tenant_id + created_by`；即使知道其他任务 ID，也不能读取或导出其他账号的数据。管理员的账号管理权限不等于跨账号查看业务任务。

侧栏“批量导出”支持选择当前账号的一个或多个任务，再选择具体门店、基础字段、画像字段、分析半径和 POI 分类。门店名称、门店地址始终固定导出；圈层字段自动命名为“500米住宅小区数量”“1公里小学数量”等。若任务只有一个分析半径会自动选中；存在多个圈层时可按需勾选。没有在某家门店所属任务中分析过的圈层或分类会留空，不会用 0 冒充结果。

可选字段包括省市区、经纬度、高德 POI ID、门店编号、品牌、匹配分、POI 总量、商圈名称与类型、商圈能级、主要/次要潜在人群、年龄段、消费能力、消费环境指数、商场环境、可信度、推断依据、限制说明、AI 摘要与可信度、失败原因等。除“门店汇总”外，可附加“POI 明细”“失败与未完成门店”“导出说明”工作表。现代 Chromium 浏览器可选择本机保存路径，其他浏览器回退到系统下载目录。

导出工作簿冻结表头、启用筛选、设置自适应列宽并保留数值类型；中文文件名使用 UTF-8 下载头，所有可能触发 Excel 公式的文本前缀都会转义。
