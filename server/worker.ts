...
import { aggregateStore, createAnalysis, generateAi, searchCandidates, searchPois, type Poi, type Row } from "./services.js";
...
async function analyzeBatch(jobId:number,tenantId:number,userId:number){
  ...
  for(const store of stores){const state=await ownerJob(jobId,tenantId,userId);if(state?.control!=="run")break;await query("UPDATE jobs SET current_store=$1,updated_at=NOW() WHERE id=$2 AND tenant_id=$3 AND created_by=$4",[store.input_name,jobId,tenantId,userId]);try{const pois=await searchPois(store,categories,radii),analysis=createAnalysis(store,pois,radii);await query("UPDATE stores SET pois_json=$1,analysis_json=$2,status='分析完成',error_message=NULL,updated_at=NOW() WHERE id=$3 AND tenant_id=$4 AND created_by=$5",[JSON.stringify(pois),JSON.stringify(analysis),store.id,tenantId,userId])}catch(error){await query("UPDATE stores SET status='分析失败',error_message=$1,updated_at=NOW() WHERE id=$2 AND tenant_id=$3 AND created_by=$4",[error instanceof Error?error.message:"分析失败",store.id,tenantId,userId])}await refreshJob(jobId,tenantId,userId,"analysis")}
...
async function processJob(job:Job){
  ...
  if(job.name==="analyze-single"){const {storeId,categories,radii}=job.data,store=(await query<Row>("SELECT * FROM stores WHERE id=$1 AND tenant_id=$2 AND created_by=$3",[storeId,tenantId,userId])).rows[0];if(!store)throw new Error("门店不存在");const pois=await searchPois(store,categories,radii),analysis=createAnalysis(store,pois,radii);await query("UPDATE stores SET pois_json=$1,analysis_json=$2,status='分析完成',updated_at=NOW() WHERE id=$3 AND tenant_id=$4 AND created_by=$5",[JSON.stringify(pois),JSON.stringify(analysis),storeId,tenantId,userId]);return {pois,analysis}}
