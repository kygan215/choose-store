...
- 正式部署：当前代码为 GitHub `main` 的 `b907f70`（`feat: add WeCom single sign-on`），生产目录 `/opt/choose-store-b907f70`；域名 `https://choose.zhekou.zirancuishipin.com/`，服务器 `47.122.104.65`。API、Web、Worker、Nginx、PostgreSQL、Redis、Backup 容器均在运行，健康接口正常。
...
- **企业微信端到端登录尚未被服务端证实成功。** 2026-09-07 09:56 的只读核验结果：配置接口 `enabled=true`，登录入口收到 1 次并返回 302，但回调为 0 次；数据库绑定用户数和 `login_wecom` 审计数均为 0。用户称“测试完毕”，接手后应先确认是否真正扫码授权并跳回系统。
...
- 企业微信登录成功后，再检查绑定用户数与审计记录；随后可删除服务器临时向导 `/root/configure-wecom.sh`。不要读取或输出 `.env.production` 和 Secret。
- 需做小批量回归：品牌库后台任务进度、完成结果进入 POI 反查、仅地址搜索、非标准 Excel 字段映射、现场照片。用户明确要求避免全量测试和大量高德调用。
- 本地工作树有多个用户自己的未跟踪目录/压缩包（`.tmp/`、PPT 依赖、`projects/`、归档包等）；不要清理、提交或覆盖。当前已跟踪代码无未提交改动，新增本文后除外。
...
1. 用可见范围内的企业微信成员，在无痕窗口完成一次扫码/OAuth；确认跳回系统且 `/api/auth/me` 已登录。
2. 只读核验企业微信绑定用户数、`login_wecom` 审计和近 20 分钟 API 错误日志；成功后删除临时配置向导。
3. 做 1–3 家门店的小批量烟雾测试，覆盖品牌库→POI 反查、地址定位、智能字段映射、照片和 Excel 导出。
4. 若通过，再更新 `README.md` 并为当前生产版本打 tag；后续发布采用新 release 目录或明确的 Git 拉取流程，保留可回滚版本和数据库备份。
