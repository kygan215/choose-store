...
- 共享品牌门店数据已在本地改为品牌、省份、城市三组多选筛选，可对组合结果执行跨页批量选择，并继续用于导出或 POI 条件反查；旧的单值接口参数保持兼容。“临时新品牌”已调整为“其余品牌”，由用户输入后直接启用并参与本次查询，不再等待管理员审核。`brand-library` 定向测试 15/15、`npm run typecheck` 和 `npm run build` 均通过。以上品牌库修正尚未部署到生产环境。
- 本地工作树有多个用户自己的未跟踪目录/压缩包（`.tmp/`、PPT 依赖、`projects/`、归档包等）；不要清理、提交或覆盖。当前另有本次品牌库修正、回归测试和本文更新尚未提交。
...
1. 审核并部署本地品牌库修正，在生产环境验证多品牌、多省份、多城市组合筛选与批量选择。
2. 更新 `README.md`，拆分“旧 Python 原型”和“当前正式版”的启动说明；确认生产回归后再为当前生产版本打 tag。后续发布采用新 release 目录或明确的 Git 拉取流程，保留可回滚版本和数据库备份。
