...
import { scoreBand, scorePoiEnvironment } from "../../scoring";
import { searchStoreCandidates } from "../../../server/store-search.js";
...
function createAnalysis(store:Row,pois:Poi[],radii:number[]){
  const primaryRadius=radii.includes(500)?500:Math.min(...radii),inside=pois.filter(poi=>poi.distance<=primaryRadius),counts:Record<string,number>={};for(const poi of inside)counts[poi.category]=(counts[poi.category]||0)+1;
  const total=inside.length,scored=scorePoiEnvironment(counts,total,primaryRadius),raw=scored.raw,created=now();
  ...
  return {id:Number(store.id),analysis_version:"cloud-v3",scoring_version:"fixed-benchmark-v1",radius_config:radii,confidence_level:total>=15?"中":total>=5?"较低":"低",business_area:{name:clean(store.district)||clean(store.city)||"门店周边",source:"高德POI代理判断",confidence:total>=15?"中":"低"},business_district_type:{type,scores:scored.typeScores,confidence:total>=15?"中":"低"},level:{level:scored.levelScore>=80?"高成熟":scored.levelScore>=60?"成熟":scored.levelScore>=40?"成长":"基础",score:scored.levelScore,mode:`${primaryRadius}米固定基准分层`},fit:{score:scored.fitScore,level:scoreBand(scored.fitScore)},competition:{score:scored.competitionPressure,level:scoreBand(scored.competitionPressure)},feature_vector:{layers:{[String(primaryRadius)]:{total,density:total,counts,nearest:{}}},level_indicators:scored.levelIndicators,fit_components:scored.fitComponents},audience_profile:{method:"基于周边设施结构的固定基准代理推断",confidence:total>=15?"中":"低",primary_groups:scored.audience.slice(0,2),age_segments:scored.audience,consumption_power:{level:scored.consumptionIndex>=80?"较高":scored.consumptionIndex>=60?"中高":scored.consumptionIndex>=40?"中等":"偏基础",index:scored.consumptionIndex,confidence:total>=15?"中":"低",basis:`商业 ${raw.commercial} 个、交通 ${raw.traffic} 个；采用边际递减评分，不等同于真实收入统计`},mall_profile:{level:(counts["购物中心"]||0)>=2?"区域商业较集中":(counts["购物中心"]||0)?"存在商场辐射":"社区商业为主",confidence:"低",sample_count:counts["购物中心"]||0,sample_names:inside.filter(p=>p.category==="购物中心").slice(0,5).map(p=>p.name),basis:"依据购物中心POI数量和名称推断，不代表官方商场评级"},summary:[`周边更可能以${scored.audience[0].label}和${scored.audience[1].label}为主要潜在人群。`,`消费环境代理指数 ${scored.consumptionIndex}/100。`],evidence:[`评分基于${primaryRadius}米内 ${total} 个有效 POI`,`各指标采用固定基准点与边际递减曲线，可跨门店横向比较`],limitations:["结果是POI环境代理推断，不是人口普查、客流或收入数据。","高德POI可能存在更新延迟和分类偏差。"]},strengths:[total>=15?"周边设施样本较充足":"已形成基础设施样本"],weaknesses:[total<15?"POI样本较少，画像可信度有限":"缺少真实订单和会员数据"],warning_messages:["评分用于同口径门店比较，请勿解释为实际人口比例或消费收入"],disclaimer:"本分析仅反映高德POI设施分布及其固定基准代理推断。",created_at:created,poi_summary:{total:pois.length,by_category:counts,by_distance:Object.fromEntries(radii.map(radius=>[`≤${radius}米`,pois.filter(p=>p.distance<=radius).length]))}};
}
