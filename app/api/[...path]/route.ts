...
type RuntimeEnv = { DB:D1; AMAP_WEB_SERVICE_KEY?:string; AMAP_REQUEST_INTERVAL_MS?:string };
...
async function exportJob(jobId:number,storeId?:number){const sql=storeId?"SELECT * FROM stores WHERE job_id=? AND id=?":"SELECT * FROM stores WHERE job_id=? ORDER BY id",statement=runtimeEnv.DB.prepare(sql).bind(...(storeId?[jobId,storeId]:[jobId])),{results}=await statement.all<Row>();const lines=[["门店名称","高德标准名称","状态","城市","区县","地址","匹配分","POI数量","商圈类型","主要年龄","消费指数","错误说明"].join(",")];for(const row of results){const analysis=parseJson<Row>(row.analysis_json,{}),audience=(analysis.audience_profile||{}) as Row,groups=Array.isArray(audience.primary_groups)?audience.primary_groups as Row[]:[],consumption=(audience.consumption_power||{}) as Row,business=(analysis.business_district_type||{}) as Row;lines.push([row.input_name,row.standard_name,row.status,row.city,row.district,row.address,row.match_score,parseJson<Poi[]>(row.pois_json,[]).length,business.type,groups.map(group=>group.age_range).join("、"),consumption.index,row.error_message].map(csvCell).join(","))}return new Response(`\uFEFF${lines.join("\r\n")}`,{headers:{"Content-Type":"text/csv; charset=utf-8","Content-Disposition":`attachment; filename="store-poi-report-${jobId}.csv"`}})}
