...
function BrandStoreLibrary(){
  const [section,setSection]=useState<"library"|"tasks"|"analysis">("library"),[options,setOptions]=useState<BrandLibraryOptions|null>(null),[cities,setCities]=useState<Array<{name:string;adcode:string}>>([]),[libraryFacets,setLibraryFacets]=useState<Array<{brand_name:string;province:string;city:string;district:string;count:number}>>([]),[jobs,setJobs]=useState<BrandLibraryJob[]>([]),[notice,setNotice]=useState(""),[busy,setBusy]=useState("");
  ...
  const [filters,setFilters]=useState({brands:[] as string[],provinces:[] as string[],cities:[] as string[],district:"",keyword:"",status:"",analyzed:""}),[page,setPage]=useState(1),[pageSize,setPageSize]=useState(50),[library,setLibrary]=useState<{rows:BrandLibraryStore[];total:number;page:number;pages:number}>({rows:[],total:0,page:1,pages:0}),[selectedIds,setSelectedIds]=useState<number[]>([]);
  ...
  const filterQuery=useMemo(()=>{const params=new URLSearchParams();for(const [key,value] of Object.entries(filters)){if(Array.isArray(value))value.forEach(item=>params.append(key,item));else if(value)params.set(key,value)}return params},[filters]);
  ...
  async function loadLibraryFacets(){try{setLibraryFacets(await request<Array<{brand_name:string;province:string;city:string;district:string;count:number}>>("/brand-library/facets"))}catch(error){setNotice(error instanceof Error?error.message:"品牌门店筛选项加载失败")}}
  async function loadJobs(){try{setJobs(await request<BrandLibraryJob[]>("/brand-library/jobs"))}catch{} }
  useEffect(()=>{const timer=window.setTimeout(()=>{void loadOptions();void loadLibraryFacets();void loadJobs()},0);return()=>window.clearTimeout(timer)},[]);
  ...
  useEffect(()=>{if(section!=="library")return;const timer=window.setTimeout(()=>void loadLibraryFacets(),0);return()=>window.clearTimeout(timer)},[section]);
  ...
  function toggleLibraryFilter(key:"brands"|"provinces"|"cities",value:string){setFilters(current=>{const values=current[key],next=values.includes(value)?values.filter(item=>item!==value):[...values,value];if(key!=="provinces")return {...current,[key]:next};const allowedCities=new Set(libraryFacets.filter(item=>!next.length||next.includes(item.province)).map(item=>item.city));return {...current,provinces:next,cities:current.cities.filter(city=>allowedCities.has(city))}});setPage(1)}
  const approvedBrands=options?.brands.filter(item=>item.status==="approved")||[],availableLibraryCities=[...new Set(libraryFacets.filter(item=>!filters.provinces.length||filters.provinces.includes(item.province)).map(item=>item.city).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"zh-CN")),selectedOnPage=library.rows.filter(row=>selectedIds.includes(row.id)).length,passedRows=(analysisData?.rows||[]).filter(row=>row.status==="符合");
...
function LoginScreen({onLogin}:{onLogin:(user:AuthUser)=>void}){
  ...
  useEffect(()=>{
    ...
    let errorTimer:number|undefined;if(message){errorTimer=window.setTimeout(()=>setError(message),0);params.delete("wecom_error");window.history.replaceState({},"",`${window.location.pathname}${params.size?`?${params.toString()}`:""}${window.location.hash}`)}return()=>{if(errorTimer)window.clearTimeout(errorTimer)}
  },[]);
